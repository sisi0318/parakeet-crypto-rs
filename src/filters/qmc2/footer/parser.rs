...
impl QMCFooterParser {
    ...
    pub fn parse_from_buffer(&self, footer: &[u8]) -> Result<(usize, Box<[u8]>), DecryptorError> {
        if footer.len() < 4 {
            return Err(DecryptorError::QMCTailBufferTooSmall);
        }

        let mut footer_magic = [0u8; 4];
        footer_magic.copy_from_slice(&footer[footer.len() - 4..]);

        let client_type = guess_type(&footer_magic)
            .ok_or_else(|| DecryptorError::QMCInvalidFooter(Box::new(footer_magic)))?;

        let (full_footer_len, encrypted_key) = match client_type {
            ClientType::AndroidSTag => return Err(DecryptorError::QMCAndroidSTag),
            ClientType::AndroidQTag => parse_footer_qtag(footer),
            ClientType::PC => parse_footer_pc(footer),
        }
        .ok_or_else(|| DecryptorError::QMCInvalidFooter(Box::new(footer_magic)))?;
        ...
        R: Read + Seek,
    {
        ...
        // known largest footer size is 0x225 (549) bytes.
        const BUFFER_FOOTER_DEFAULT_READ_LEN: usize = 0x400;

        let read_len = std::cmp::min(file_len, BUFFER_FOOTER_DEFAULT_READ_LEN);
        let mut buffer_full_footer = vec![0u8; read_len];
        input.seek(SeekFrom::End(-(read_len as i64)))?;
        input.read_exact(&mut buffer_full_footer)?;

        self.parse_from_buffer(&buffer_full_footer[..])
    }
}

#[cfg(test)]
mod tests {
    use super::super::decrypt_key::make_simple_key;
    use super::*;
    use crate::interfaces::DecryptorError;
    use std::io::Cursor;
